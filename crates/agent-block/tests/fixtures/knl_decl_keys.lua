...
-- per table the kernel's own record declares — the module itself (its
-- functions and tables), `knl.views`, `knl.shapes`, `knl.Outcome`.
-- tests/e2e_knl_decl_drift.rs reads that record out of `blocks/lib/knl/
-- init.tl` and holds it to these lines: a name declared that the module does
-- not export is a declaration that has drifted.
