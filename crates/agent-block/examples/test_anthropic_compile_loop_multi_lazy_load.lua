-- test_anthropic_compile_loop_multi_lazy_load.lua
--
...
--
-- Does NOT call the real Anthropic API (offline / no cost).
-- Acceptance criteria:
...
if in_test_framework then
    ...
    -- agent module mock: returns an LLM context pointing at "anthropic" so that
    -- run_loop does not fall back to "openai" (which errors on multi-file).
    package.preload["agent"] = package.preload["agent"]
        or function()
            return {
                _llm_ctx_top = function()
                    return { provider = "anthropic", api_key = "mock-key" }
                end,
            }
        end
...
describe("compile_loop multi-file lazy load e2e", function()
    ...
    it("(alpha/beta/gamma) lazy load path verification — 10 iters mock", function()
        ...
        for i, path in ipairs(TARGET_FILES) do
            write_file(path, "-- lazy_file_" .. i .. "_v0\n")
        end
        ...
        local function make_sr_block(file_idx, call_tag)
            ...
            local search_line
            if cur_ver == 1 then
                search_line = "-- lazy_file_" .. file_idx .. "_v0\n"
            else
                search_line = "-- lazy_file_" .. file_idx .. "_v" .. (cur_ver - 1) .. "_c" .. (cur_ver - 1) .. "\n"
            end
            local replace_line = "-- lazy_file_" .. file_idx .. "_v" .. cur_ver .. "_c" .. call_tag .. "\n"
        ...
        local td = compile_loop.make({
            runner = lazy_runner,
            llm = { provider = "anthropic", api_key = "mock-key" },
        ...
        local handle_ok, result_json = pcall(td.handler, {
            spec = "Add version comments to each lazy file.",
            target_files = TARGET_FILES,
        })
    ...
    it("stagnation regression — forced-fail runner triggers stagnation give-up", function()
        local TF_A = "/tmp/stag_reg_a.lua"
        local TF_B = "/tmp/stag_reg_b.lua"
        write_file(TF_A, "-- stag_a_v0\n")
        write_file(TF_B, "-- stag_b_v0\n")
