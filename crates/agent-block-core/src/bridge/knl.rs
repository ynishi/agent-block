...
/// The `knl_types` Teal declaration, as source text: the same types
/// [`lshape_module_source`] renders as lshape, rendered as a `.d.tl` for the
/// checker a module written in Teal is held to.
///
/// One source, two renderings. lshape is what `knl` asserts at run time in
/// dev mode; this is what `htl check` and `include_tl!` read a `.tl`'s
/// `require("knl_types")` against. Both come off [`types::declared`], so a
/// Rust type that moves moves both, and neither can be edited into saying
/// something the other does not.
///
/// The tree holds a copy (`blocks/lib/knl_types.d.tl`) for the checker that
/// runs at `cargo build` and in CI, which reads the project's own files and
/// nothing a host wrote at start; a test pins that copy to this function
/// (`knl_types_declaration_in_the_tree_is_current`), the way htl pins the
/// `.d.tl` its own macros write. The host writes this same text beside
/// `host_types.d.tl` at start (`host::teal_declarations`), so a project's
/// `.tl` is checked against the binary that runs it, whatever the tree said.
///
/// What Teal cannot say is said the nearest way: a union of two table types
/// is `any` (Teal refuses to discriminate them), and a field's own object or
/// enum is a record or enum nested in the parent, named after the field.
/// A closed table ([`types::Strict::Closed`]) needs no marker — a Teal
/// record refuses a field it does not declare.
pub fn teal_module_source() -> String {
    static SOURCE: std::sync::OnceLock<String> = std::sync::OnceLock::new();
    SOURCE.get_or_init(build_teal_module_source).clone()
}

fn build_teal_module_source() -> String {
    let mut out = String::from(
    ...
    for (name, schema, _strict) in types::declared() {
        out.push_str(&teal::declaration(name, &schema, 1));
    }
    out.push_str("end\nreturn knl_types\n");
    out
}

/// schema-bridge IR → Teal declaration text.
mod teal {
    use schema_bridge::{Field, Schema};

    /// `name`'s declaration at `depth` (four spaces per level): a `record`
    /// for an object, an `enum` for a closed set of strings, a `type` alias
    /// for anything else.
    pub fn declaration(name: &str, schema: &Schema, depth: usize) -> String {
        let pad = "    ".repeat(depth);
        match schema {
            Schema::Object(fields) => {
                let mut out = format!("{pad}record {name}\n");
                for field in fields {
                    out.push_str(&field_lines(field, depth + 1));
                }
            ...
            other => format!("{pad}type {name} = {}\n", type_text(other)),
        }
    }

    /// One field of a record: an object or an enum of its own is declared
    /// nested, named after the field, and the field refers to it.
    fn field_lines(field: &Field, depth: usize) -> String {
        let pad = "    ".repeat(depth);
        match &field.schema {
            Schema::Object(_) | Schema::Enum(_) => {
                let nested = pascal(&field.name);
                let mut out = declaration(&nested, &field.schema, depth);
                out.push_str(&format!("{pad}{}: {nested}\n", key(&field.name)));
                out
            }
            // An array of objects: the item is declared nested too, as the
            // field's `Entry`, so the field is `{Entry}` rather than `{any}`.
            Schema::Array(item) if matches!(**item, Schema::Object(_) | Schema::Enum(_)) => {
                let nested = format!("{}Entry", pascal(&field.name));
                let mut out = declaration(&nested, item, depth);
                out.push_str(&format!("{pad}{}: {{{nested}}}\n", key(&field.name)));
                out
            }
            other => format!("{pad}{}: {}\n", key(&field.name), type_text(other)),
        }
    }

    /// A Teal type for a schema that stands in a type position.
    fn type_text(schema: &Schema) -> String {
        match schema {
            Schema::String => "string".into(),
            Schema::Number => "number".into(),
            Schema::Integer => "integer".into(),
            Schema::Boolean => "boolean".into(),
            Schema::Null => "nil".into(),
            Schema::Any => "any".into(),
            Schema::Array(item) => format!("{{{}}}", type_text(item)),
            Schema::Record { key, value } => {
                format!("{{{}:{}}}", type_text(key), type_text(value))
            }
            Schema::Tuple(items) => {
                let inner: Vec<String> = items.iter().map(type_text).collect();
                format!("{{{}}}", inner.join(", "))
            }
            Schema::Ref(name) => name.clone(),
            Schema::Union(variants) => {
                // Teal discriminates a union by the Lua type of its members,
                // so two table types in one union is a union it refuses.
                let tables = variants.iter().filter(|v| is_table(v)).count();
                if tables > 1 {
                    return "any".into();
                }
                ...
                for v in variants {
                    let part = match v {
                        // An object or enum with no name of its own has no
                        // place to be declared from a type position.
                        Schema::Object(_) | Schema::Enum(_) => "any".to_string(),
                ...
                // `any` absorbs the rest: a union with it in is it.
                if parts.iter().any(|p| p == "any") {
                    return "any".into();
                }
                parts.join(" | ")
            }
            // Reached only through a union (a field's own object / enum is
            // declared by `field_lines`).
            Schema::Object(_) | Schema::Enum(_) => "any".into(),
        }
    }

    fn is_table(schema: &Schema) -> bool {
        matches!(
            schema,
            Schema::Array(_)
                | Schema::Object(_)
    ...
    /// A record key: bare when Teal takes it bare, bracketed and quoted
    /// otherwise (a keyword, or a name with a character an identifier
    /// cannot carry).
    ...
    /// `snake_case` → `PascalCase`, for a nested declaration's name.
...
#[cfg(test)]
mod tests {
    /// `blocks/lib/knl_types.d.tl` is what this crate renders, byte for
    /// byte. The tree copy is for the checker at `cargo build` and in CI;
    /// the host hands out its own render at start. `AGENT_BLOCK_WRITE_DTS=1`
    /// makes this test write the file instead of comparing, which is how it
    /// is regenerated after the Rust types move.
    #[test]
    fn knl_types_declaration_in_the_tree_is_current() {
        // Two files, one generator each: the Teal declaration for the
        // checker, and the lshape module for the two runners that have no
        // host to build it at start (`htl test`, `lua-spec-runner`) — the
        // same text the host embeds as `knl_types`, so a spec that reaches
        // the kernel there reaches the same types.
        let lib = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("blocks/lib");
        let files = [
            ("knl_types.d.tl", super::teal_module_source()),
            ("knl_types.lua", super::lshape_module_source()),
        ];
        let write = std::env::var_os("AGENT_BLOCK_WRITE_DTS").is_some();
        for (name, rendered) in &files {
            let path = lib.join(name);
            if write {
                std::fs::write(&path, rendered).expect("write the generated module");
                continue;
            }
            let in_tree = std::fs::read_to_string(&path).unwrap_or_default();
            assert!(
                &in_tree == rendered,
                "{} is not what bridge/knl.rs renders — regenerate it:\n  \
