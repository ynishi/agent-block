...
mod tests {
    ...
    fn knl_types_declaration_in_the_tree_is_current() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("blocks/lib/knl_types.d.tl");
        let rendered = super::teal_module_source();
        if std::env::var_os("AGENT_BLOCK_WRITE_DTS").is_some() {
            std::fs::write(&path, &rendered).expect("write knl_types.d.tl");
            return;
        }
