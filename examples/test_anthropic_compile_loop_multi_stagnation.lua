...
local function lua_runner_forced_fail(file_paths)
    ...
    log.info("forced-fail runner call #" .. runner_call_count
        .. " (input_type=" .. type(file_paths) .. " count_or_strlen=" .. n .. ")")
    return {
        ok = false,
        stdout = "",
...
local result = agent.run({
    ...
    prompt         = string.format(
        "Use the compile_loop tool ONCE. Files: %s, %s.\n%s",
        TARGET_A, TARGET_B, SPEC
    ),
})

log.info("=== RESULT === agent.ok=" .. tostring(result.ok))
log.info("runner call count = " .. runner_call_count
    .. " (max_iters=" .. MAX_ITERS .. ")")
...
log.info("tool_output: ok=" .. tostring(tool_output.ok)
...
local summary_says_max = type(tool_output.summary) == "string"
    and tool_output.summary:find("max_iters", 1, true) ~= nil

log.info("checks: tool_ok_false=" .. tostring(tool_ok_false)
    .. " iters_at_max=" .. tostring(iters_at_max)
    .. " runner_bounded=" .. tostring(runner_bounded)
    .. " summary_says_max=" .. tostring(summary_says_max))
