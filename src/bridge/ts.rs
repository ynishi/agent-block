...
/// - **raw** (`agg = None`): `SELECT ts, value, tags … ORDER BY ts LIMIT ? OFFSET ?`
...
fn build_query_sql(
    ...
    match agg {
        ...
        None => {
            let sql =
                format!("SELECT ts, value, tags FROM ts {where_clause} ORDER BY ts{limit_clause}");
            Ok(sql)
        }
            ...
            match bucket_ms {
                ...
                None => {
                    if agg_name == "last" {
                        ...
                        let sql = format!(
                            "SELECT value, tags, ts FROM ts {where_clause} ORDER BY ts DESC LIMIT 1"
                        );
                        Ok(sql)
                    } else {
